	...
	class Directory_Core_Main extends Directory_Core {
		...
		function process_page_requests() {
			...
			if ( is_page( $this->my_listings_page_id ) ) {
				if ( isset( $_POST['action'], $_POST['_wpnonce'] ) && 'delete_listing' == $_POST['action'] && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'action_verify' ) ) {
			...
			elseif ( is_page( $this->add_listing_page_id ) || is_page( $this->edit_listing_page_id ) ) {
				...
				if ( isset( $_POST['update_listing'], $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'verify' ) ) {


					$post_status = isset($_POST['listing_data']['post_status']) ? sanitize_key(wp_unslash($_POST['listing_data']['post_status'])) : '';
					$draft   = $post_status === 'draft';
					$post_id = $this->update_listing( $_POST );
					if (!empty($this->last_profile_validation_errors)) {
						$draft = true;
						$_POST['listing_data']['post_status'] = 'draft';
						$error = __('Bitte vervollstaendige Dein Branchenprofil. Solange Pflichtfelder fehlen, bleibt der Eintrag als Entwurf gespeichert.', $this->text_domain);
						$error .= '<br />- ' . implode('<br />- ', array_map('esc_html', $this->last_profile_validation_errors));
