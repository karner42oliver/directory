...
if (!class_exists('Directory_Core')):
    class Directory_Core
    {
        ...
        public $last_profile_validation_errors = array();
        ...
        // Seiten-ID für meine Gutschriften
        public $my_credits_page_id;

        // Seiten-Slug für meine Gutschriften
        ...
        /**
         * Registers Directory post type and taxonomies natively.
         *
         * @return void
         */
        function register_content_types() {
            if (!post_type_exists('directory_listing')) {
                register_post_type('directory_listing', array(
                    ...
                    'labels' => array(
                        'name' => __('Einträge', $this->text_domain),
                        'singular_name' => __('Eintrag', $this->text_domain),
                        'add_new' => __('Neuen Eintrag hinzufügen', $this->text_domain),
                        'add_new_item' => __('Neuen Eintrag hinzufügen', $this->text_domain),
                        'edit_item' => __('Bearbeite Eintrag', $this->text_domain),
                        'new_item' => __('Neuer Eintrag', $this->text_domain),
                        'view_item' => __('Eintrag ansehen', $this->text_domain),
                        'search_items' => __('Suche Einträge', $this->text_domain),
                        'not_found' => __('Keine Einträge gefunden', $this->text_domain),
                        'not_found_in_trash' => __('Keine Einträge im Papierkorb gefunden', $this->text_domain),
                    ),
                ));
            }

            if (!taxonomy_exists('listing_tag')) {
                register_taxonomy('listing_tag', array('directory_listing'), array(
                    'public' => true,
                    'hierarchical' => false,
                    ...
                    'labels' => array(
                        'name' => __('Eintrags-Tags', $this->text_domain),
                        'singular_name' => __('Eintrags-Tag', $this->text_domain),
                        'search_items' => __('Suche Eintrags-Tags', $this->text_domain),
                        'popular_items' => __('Beliebte Eintrags-Tags', $this->text_domain),
                        'all_items' => __('Alle Eintrags-Tags', $this->text_domain),
                        'edit_item' => __('Bearbeite Eintrags-Tag', $this->text_domain),
                        'update_item' => __('Aktualisiere Eintrags-Tags', $this->text_domain),
                        'add_new_item' => __('Neuen Eintrags-Tag hinzufügen', $this->text_domain),
                        'new_item_name' => __('Neuer Eintrags-Tag-Name', $this->text_domain),
                        'add_or_remove_items' => __('Eintrags-Tags hinzufügen oder entfernen', $this->text_domain),
            ...
            if (!taxonomy_exists('listing_category')) {
                register_taxonomy('listing_category', array('directory_listing'), array(
                    'public' => true,
                    'hierarchical' => true,
                    ...
                    'capabilities' => array('assign_terms' => 'edit_listings'),
                    'labels' => array(
                        'name' => __('Eintragskategorien', $this->text_domain),
                        'singular_name' => __('Eintragskategorie', $this->text_domain),
                        'search_items' => __('Such Eintragskategorien', $this->text_domain),
                        'popular_items' => __('Beliebte Eintragskategorien', $this->text_domain),
                        'all_items' => __('Alle Eintragskategorien', $this->text_domain),
                        'parent_item' => __('Eltern-Kategorie', $this->text_domain),
                        'edit_item' => __('Bearbeite Eintragskategorie', $this->text_domain),
                        'update_item' => __('Aktualisiere Eintragskategorie', $this->text_domain),
                        'add_new_item' => __('Neue Eintragskategorie hinzufügen', $this->text_domain),
                        'new_item_name' => __('Neue Eintragskategorie', $this->text_domain),
                        'parent_item_colon' => __('Eltern-Kategorie:', $this->text_domain),
                        'add_or_remove_items' => __('Eintragskategorien hinzufügen oder entfernen', $this->text_domain),
                    ),
                ));
            }
        }
        ...
        function create_default_pages()
        {
            ...
            if (empty($page_id)) {
                ...
                $args = array(
                    'post_title' => __('Einträge', $this->text_domain),
                    'post_status' => 'publish',
            ...
            if (empty($page_id)) {
                /* Construct args for the new post */
                $args = array(
                    'post_title' => __('Meine Einträge', $this->text_domain),
                    'post_status' => 'publish',
        ...
        function update_listing($params)
        {
            ...
            $listing_data = (isset($params['listing_data']) && is_array($params['listing_data'])) ? $params['listing_data'] : array();
            $post_status = isset($listing_data['post_status']) ? sanitize_key($listing_data['post_status']) : 'draft';
            $this->last_profile_validation_errors = array();

            /* Construct args for the new post */
            $args = array(
                /* If empty ID insert Listing instead of updating it */
                'ID' => isset($listing_data['ID']) ? intval($listing_data['ID']) : 0,
                'post_title' => isset($listing_data['post_title']) ? wp_strip_all_tags($listing_data['post_title']) : '',
                'post_content' => isset($listing_data['post_content']) ? $listing_data['post_content'] : '',
                'post_excerpt' => isset($listing_data['post_excerpt']) ? $listing_data['post_excerpt'] : '',
            ...
            if (!empty($post_id)) {
                ...
                $profile_input = isset($params['dr_business']) ? wp_unslash($params['dr_business']) : array();
                $profile = $this->sanitize_listing_business_profile($profile_input, $post_id);
                ...
                if (!empty($errors)) {
                    $this->last_profile_validation_errors = $errors;
        ...
        /**
         * Returns validation rules for listing profiles.
         *
         * @param array $category_slugs Selected category slugs.
         * @return array
         */
        ...
        /**
         * Validates listing profile fields.
         *
        ...
        /**
         * Sanitizes listing profile input.
         *
         * @param array $submitted Submitted profile fields.
         * @param int   $post_id Listing post ID.
         * @return array
         */
        function sanitize_listing_business_profile($submitted, $post_id = 0) {
            ...
            $old = ($post_id > 0) ? get_post_meta($post_id, '_dr_business_profile', true) : array();
        ...
        {
            extract(shortcode_atts(array(
                'text' => __('Einträge', $this->text_domain),
        ...
        {
            extract(shortcode_atts(array(
                'text' => __('Eintrag hinzufügen', $this->text_domain),
                'view' => 'both', //loggedin, loggedout, both
            ), $atts));
        ...
        {
            extract(shortcode_atts(array(
                'text' => __('Meine Einträge', $this->text_domain),
        ...
        /**
         * Add directory_listing to the post types supporting thumbnails.
         *
         */
