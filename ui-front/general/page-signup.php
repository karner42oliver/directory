...
<form action="#" method="post"  class="dr-checkout">
	...
	<table id="billing-type" class="form-table">
		...
		<tr>
			<th <?php do_action( 'billing_invalid' ); ?>><label for="billing"><?php _e( 'Guthaben kaufen', $this->text_domain ) ?></label></th>
			<td>
			...
			<label for="type_recurring"><?php echo (empty( $opset['recurring_name'] ) ) ? '' :$opset['recurring_name']; ?></label></th>
				<td>
					...
					<span>
						...
						$bastr .= ($opset['billing_frequency'] > 1) ? __(' Zeitraum', $this->text_domain) : '';
						echo $bastr;
						?>
					</span>
		...
		<strong><?php _e( 'Das Posten von Inseraten ist kostenlos, wenn Du angemeldet bist' ); ?></strong>
		<?php else: ?>
		<form action="#" method="post"  class="checkout">
			...
			<table class="form-table">
				...
				<tr>
					<th <?php do_action('pm_invalid'); ?>>
						<label for="pmethod_cc"><?php _e( 'Kreditkarte', $this->text_domain ); ?></label>
					</th>
					<td>
		...
		<form action="#" method="post" class="checkout" id="confirm_payment">
			...
			<table id="cc-user-details" class="form-table">
				...
				<tr>
					<td><label for="last-name"><?php _e( 'Nachname', $this->text_domain ); ?>:</label></td>
				...
				<?php if(! $this->use_free): ?>
				<tr>
					<td><?php _e( 'Gesamtsumme', $this->text_domain ); ?>:</td>
					<td>
						<strong><?php echo $_SESSION['cost']; ?> <?php echo (empty($oppaypal['currency']) ) ? 'EUR' : $oppaypal['currency']; ?></strong>
				...
				<tr>
					<td><label for="exp_date"><?php _e( 'Ablaufdatum', $this->text_domain ); ?>:</label></td>
					<td>
		...
		<form action="" method="post" class="checkout">
			...
			<table>
				...
				<tr>
					<td><label><?php _e( 'E-Mail-Addresse', $this->text_domain ); ?>:</label></td>
				...
				<tr>
					<td><label><?php _e('Gesamtsumme', $this->text_domain); ?>:</label></td>
					<td>
						<strong><?php printf('%s %s', $cc['total_amount'], empty($cc['currency_code']) ? 'EUR' : $oppaypal['currency']); ?></strong>
					</td>
				</tr>
			</table>

			<div class="submit">
				<input type="submit" name="confirm_payment_submit" value="Bestätige Zahlung" />
			</div>
...
<?php /* Success */ ?>
		...
		<div class="dp-submit-txt"><?php _e( 'Vielen Dank für Deinen Einkauf. Transaktion erfolgreich verarbeitet!', $this->text_domain ); ?></div>
		<span class="dp-submit-txt"><?php _e( 'Du kannst zu Deinem Profil gehen und Deine persönlichen Daten überprüfen/ändern, oder Du kannst direkt zur Seite für die Einreichung von Verzeichniseinträgen gehen.', $this->text_domain ); ?></span>
		<br />
		...
		<?php echo do_shortcode('[dr_my_listings_btn text="' . __('Gehe zu Deinen Einträgen', $this->text_domain) . '" view="always"]'); ?>


		<form id="go-to-profile-su" action="#" method="post">
			<input type="submit" name="redirect_profile" value="Gehe zum Profil" />
		</form>
		<br class="clear" />
		...
		<form action="" method="post" class="checkout">
			...
			<table>
				...
				<tr>
					<td><label><?php _e('Gesamtsumme', $this->text_domain); ?>:</label></td>
					<td>
						<strong><?php echo $cc['total_amount']; ?> <?php echo (empty($cc['currency_code']) ) ? 'EUR' : $oppaypal['currency']; ?></strong>
					</td>
				</tr>
			</table>
			<div class="submit">
