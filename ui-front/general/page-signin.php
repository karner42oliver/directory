...
<div id="login-register-password">
	...
	<ul class="dr_tabs">
		<li class="dr_active"><a href="#tab1_login"><?php _e('Anmeldung', $this->text_domain); ?></a></li>
		<?php if($can_register): ?>

		<?php if(isset($bp) ): ?>
		<li><a href="<?php echo esc_url( site_url('wp-login.php?action=register', 'login_post') ); ?>"><?php _e('Account registrieren', $this->text_domain); ?></a></li>
		<?php else: ?>
		<li><a href="#tab2_login"><?php _e('Neues Konto', $this->text_domain); ?></a></li>
		<?php endif; ?>
	...
	<div class="dr_tab_container">
		...
		<div id="tab1_login" class="dr_tab_content">
			...
			<p><?php _e('Überprüfe Deine E-Mail auf das Passwort und kehre dann zum Anmelden zurück.', $this->text_domain); ?></p>
			...
			<h3><?php _e('Hast Du einen Account?', $this->text_domain); ?></h3>
			<p><?php _e('Du mussen Dich anmelden, um den Inhalt dieser Seite anzuzeigen.', $this->text_domain); ?></p>
			<p><?php _e('Anmelden oder Registrieren! Es ist schnell &amp; <em>kostenlos!</em>', $this->text_domain); ?></p>

			<?php endif; ?>

			<form method="post" action="<?php echo wp_login_url(); ?>" class="wp-user-form">
				<div class="username">
					<label for="user_login"><?php _e('Nutzername', $this->text_domain); ?>: </label>
				...
				<div class="login_fields">
					<div class="rememberme">
						<label for="rememberme">
							<input type="checkbox" name="rememberme" value="forever" checked="checked" id="rememberme" tabindex="13" /> <?php _e('Erinnere dich an mich', $this->text_domain); ?>
						</label>
					</div>
					<?php do_action('login_form'); ?>
					<input type="submit" name="user-submit" value="<?php _e('Anmeldung', $this->text_domain); ?>" tabindex="14" class="user-submit" />
		...
		<div id="tab2_login" class="dr_tab_content" style="display:none;">
			...
			<form method="post" id="register_frm" action="<?php echo network_site_url('wp-signup.php', 'login_post') ?>" class="wp-user-form">
				...
				<div class="username">
					<label for="user_name"><?php _e('Nutzername', $this->text_domain); ?>: </label>
				...
				<form method="post" id="register_frm" action="<?php echo esc_url( site_url('wp-login.php?action=register', 'login_post') ); ?>" class="wp-user-form">
					...
					<div class="password">
						<label for="user_email"><?php _e('Deine E-Mail', $this->text_domain); ?>: </label>
					...
					<div class="login_fields">
						...
						<input type="submit" name="user-submit" value="<?php _e('Anmelden!', $this->text_domain); ?>" class="user-submit" tabindex="104" />
						<?php if($register == true): ?>
						<p><?php _e('Überprüfe Deine E-Mail auf das Passwort!', $this->text_domain); ?></p>
						````````````<?php endif; ?>
			...
			<div id="tab3_login" class="dr_tab_content" style="display:none;">
				<h3><?php _e('Etwas verloren?', $this->text_domain); ?></h3>
		...
		<div class="sidebox">
			...
			<div class="userinfo">
				...
				<p>
					<a href="<?php echo wp_logout_url('index.php'); ?>"><?php _e('Ausloggen', $this->text_domain); ?></a> |
	...
	<script type="text/javascript">
		(function($) {
			...
			$("#register_frm").submit(function(){
				if( ! $('#tos_agree').prop('checked') ) {alert("<?php echo __('Bitte akzeptiere die Geschäftsbedingungen', $this->text_domain); ?>"); return false;}
			});
			<?php endif; ?>
